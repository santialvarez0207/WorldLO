...
export class CExamenesComponent implements OnInit {
  ...
  ngOnInit(): void {
    //es lo primero que se ejectua
    this.Agregar(); // agrega la primera pregunta
    var a = localStorage.getItem("name");
    this.usuarioService.getusuarios().subscribe((res) => {
      // esta parte sirve para verificar si el usuario esta en la base de datos
    ...
    let boton = document.getElementById("Ms"); //oculta boton de mensajes
    boton.style.visibility = "hidden";
  }

  ngOnDestroy() {
    let boton = document.getElementById("Ms"); //vuelve a colocar lo de mensaje
    boton.style.visibility = "visible";
  }

  onPhotoSelected(event: HtmlInputEvent): void {
    if (event.target.files && event.target.files[0]) {
      this.file = <File>event.target.files[0]; //carga el archivo 0
      // image preview
      const reader = new FileReader();
      reader.onload = (e) => (this.photoSelected = reader.result);
      reader.readAsDataURL(this.file); //muestra la imagen
    }
  }

  envioExam() {
    //luego de haber cargado todas las preguntas ser anexadas al examen se envia el examen a la base de datos
    var G: Array<string> = [];
    var a: string;
    var res: string;
    var b;
    ...
    } //toma todas las imagenes y las guarda en el arreglo

    this.imagenservice
      .postImagenes(imagenes, this.imag - 1)
      .subscribe((res) => {
        ...
        this.ExamService.postExam(
          ...
          var link = "./Examenes/" + aqui._id;
          document.getElementById("id").onclick = function tata() {
  ...
  Agregar() {
    // esta funcion sirve para agregar preguntas al cuestionario
    ...
    for (var ii = 0; ii <= 3; ii++) {
      ...
      n.appendChild(b); //opciones de respuesta
    ...
    var option = document.createElement("option"); //para selecionar cual de todas quiere
    option.value = "1";
    option.text = "1";
  ...
  Eliminar() {
    //esta sirve para eliminar preguntas del cuestionario
  ...
  ordenfuction(a: number, b: string) {
    if (a == 1) {
      this.orden[this.ii] = b; // lo agrega para saber cual es el orden
      this.ii++;
    } else {
      console.log("se metio loco");
      ...
      for (var i = this.ii; i >= 0; i--) {
        if (this.orden[i] == b && state == false) {
          ...
          this.ii--;
          this.orden.length = this.orden.length - 1;
          state = true;
        }
      }
    }
