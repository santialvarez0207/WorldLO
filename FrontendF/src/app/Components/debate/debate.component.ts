...
export class DebateComponent implements OnInit {
  ...
  constructor(private socketService: SocketsService,private debateService:DebateService) { }
  ...
  private punta():void{    
    ...
    navigator.mediaDevices.getUserMedia({
      ...
      let video=<HTMLVideoElement>document.getElementById("previsualizacion") 
      video.srcObject=media
      video.style.display="none"
      let asdasd=<HTMLAudioElement>document.getElementById("previsualizacion") 
...
crear(){
  ...
  this.debateService.postDebate(nombre.value,localStorage.getItem("id"),password.value).subscribe(res=>{
let y = res as Debate
this.id= y._id
...
unirse(){
  ...
  this.debateService.getDebates().subscribe(res=>{
    ...
    if(y[i].name == nombre.value && y[i].password==password.value &&  y[i]._id==id.value){
      this.id= y[i]._id
      this.tipo="2"
      this.punta()
      this.envio()
