...
export class ExamcVisualiomponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    }); // funcione partes de materialize
  }
...
*/
  ...
  comprobar(a: string) {
    // verifica si el usuario es el creador del examen
    this.ExamService.getExam(a).subscribe((res) => {
      ...
      this.usuarioService
        ...
        .subscribe((res) => {
          var usuario = res as Usuario;
          if (
            localStorage.getItem("id") == this.b.usserid ||
            usuario.tipeuser == "2"
          ) {
            this.tabla(); //crear tabla
          } else {
  ...
  delete() {
    //si se preciona el boton , eliminar el cuestionario
  ...
  preguntas(a: string, b: Array<string>, i: number) {
    //dibujar las preguntas
  ...
  Tomarvalores() {
    //para cuando se de enter se pida todos los valores
    let prueba;
    var x: Array<string> = [],
      acum: number = 0,
      punt: number = 0;
    ...
    this.b.respondidas[this.b.respondidas.length] = {
      respuestas: x,
      nombre: localStorage.getItem("name"),
      puntuacion: punt,
    };
    this.ExamService.putExam(
      this.b._id,
      this.b.title,
      this.b.intro,
      this.b.usserid,
      this.b.ussename,
      this.b.configuracion,
      this.b.questionid,
      this.b.imagen,
      this.b.orden,
      this.b.respondidas
  ...
  } //se envia con las respuestas selecionadas

  contenidos(
    questionid: Array<{
      //es para crear imganes y preguntas
    ...
    for (var i = 0; i <= d.length - 1; i++) {
      if (d[i] == "1") {
        this.preguntas(
          questionid[this.conta].pregunta,
          questionid[this.conta].preguntas,
          this.conta
        );
        this.conta++;
      } else if (d[i] == "2") {
