...
export class Gcreatecomponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    var a = localStorage.getItem("name"); // cache
    ...
    if (a == "") {
      ...
      window.location.replace("./Usuarios"); //redirigir
    }
    // verifica si el usuario es apto
    let boton = document.getElementById("Ms"); //ocultar boton de chat
  ...
  addpages(title: string, intro: string) {
  ...
  verificacion() {
    // verificar si cumple los parametros
    ...
    intro = GG.value;
    this.state = false; //falso: si es apto ... verdadero: no apto
    if (intro.length > 700) {
      M.toast({ html: "intro:  max 700" });
      this.state = true;
    }
    if (this.state == false) {
      this.addpages(title, intro); //Crea la pagina en la base de datos
    }
  }
}
