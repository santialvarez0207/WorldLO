...
export async function getImageness(
  req: Request,
  res: Response
): Promise<Response> {
...
export async function createImagenes(
  req: Request,
  res: Response
): Promise<Response> {
...
export async function deleteImagenes(
  req: Request,
  res: Response
): Promise<Response> {
  const { id } = req.params; //toma la id
...
export async function updateImagenes(
  req: Request,
  res: Response
): Promise<Response> {
  const { id } = req.params;
  const { name, password, mail, tipeimagen, like1, like2, like3 } = req.body;
  const updatedImagenes = await Imagenes.findByIdAndUpdate(id, {
    name,
    password,
    mail,
    tipeimagen,
